        ...
        <div className="bg-card p-6 rounded-2xl shadow-soft border border-border">
            <header className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-4">
                    {view === 'month' ? (
                        <>
                            <button onClick={() => changeMonth(-1)} className="p-2 rounded-full hover:bg-muted"><ChevronLeftIcon /></button>
                            <h3 className="text-xl font-bold text-center w-48">
                                {currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}
                            </h3>
                            <button onClick={() => changeMonth(1)} className="p-2 rounded-full hover:bg-muted"><ChevronRightIcon /></button>
                        </>
                    ) : (
                        <>
                            <button onClick={() => changeWeek(-1)} className="p-2 rounded-full hover:bg-muted"><ChevronLeftIcon /></button>
                            ...
                            <button onClick={() => changeWeek(1)} className="p-2 rounded-full hover:bg-muted"><ChevronRightIcon /></button>
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden sm:flex items-center gap-2">
                         {view === 'week' && 
                            <>
                                <button onClick={() => changeWeek(-1)} className="px-3 py-1.5 text-sm font-semibold rounded-lg text-muted-foreground hover:bg-muted">Previous Week</button>
                                <button onClick={() => changeWeek(1)} className="px-3 py-1.5 text-sm font-semibold rounded-lg text-muted-foreground hover:bg-muted">Next Week</button>
                            </>
                        }
                        <button onClick={handleGoToToday} className="px-3 py-1.5 text-sm font-semibold rounded-lg text-muted-foreground hover:bg-muted border border-border">Today</button>
                    </div>

                    <div className="flex bg-muted rounded-lg p-1">
                        <button
                            onClick={() => setView('month')}
                            className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${view === 'month' ? 'bg-background shadow-sm' : 'text-muted-foreground'}`}
                        >
                            Month View
                        </button>
                        <button
                            onClick={() => setView('week')}
                            className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${view === 'week' ? 'bg-background shadow-sm' : 'text-muted-foreground'}`}
                        >
                            Week View
                        </button>
                    </div>
                ...
                <div>
                    <div className="grid grid-cols-7 text-center">
                        {weekDates.map(date => {
                            ...
                            return (
                                <div key={date.toISOString()} className={`py-2 border-b-2 border-border ${isToday ? 'bg-yellow-500/10' : ''}`}>
                                    <p className="text-xs text-muted-foreground">{date.toLocaleDateString('en-US', { weekday: 'short' })}</p>
                                    <div className="flex items-center justify-center gap-1 mt-1">
                                        <p className={`text-2xl font-bold ${isToday ? 'text-yellow-600 dark:text-yellow-300' : 'text-foreground'}`}>{date.getDate()}</p>
                                        {isToday && (
                                            <span className="text-xs font-bold text-yellow-800 bg-yellow-200 dark:bg-yellow-400/20 dark:text-yellow-300 rounded-full px-2 py-0.5">Today</span>
                                        )}
                                    </div>
                                </div>
                            );
                        ...
                        {weekDates.map(date => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={`p-2 border-r border-border overflow-y-auto space-y-2 transition-colors thin-scrollbar
                                        ${dragOverDate === dateString ? 'bg-primary/20' : ''} 
