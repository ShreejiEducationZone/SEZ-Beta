...
You must be able to understand and respond in English, Hindi, and Gujarati.`;
    ...
    return (
        <div className="absolute bottom-0 left-0 right-0 h-[80%] max-h-[600px] bg-card/80 backdrop-blur-lg border-t border-border shadow-2xl rounded-t-2xl flex flex-col transition-transform transform translate-y-0 duration-300 ease-in-out">
            {/* Header */}
            <header className="flex items-center justify-between p-4 border-b border-border flex-shrink-0">
                <h3 className="text-lg font-semibold text-primary">AI Subject Builder</h3>
            ...
            {/* Chat Messages */}
            <div ref={chatContainerRef} className="flex-grow p-4 overflow-y-auto space-y-4 thin-scrollbar">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                        {msg.role === 'model' && <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0"><RobotIcon className="h-5 w-5 text-primary"/></div>}
                        <div className={`whitespace-pre-wrap max-w-sm md:max-w-md rounded-2xl px-4 py-2.5 text-sm ${msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-br-lg' : 'bg-muted text-card-foreground rounded-bl-lg'}`}>
                            {msg.text}
                        </div>
                    </div>
                ))}
                 {isLoading && (
                    <div className="flex gap-3">
                        <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0"><RobotIcon className="h-5 w-5 text-primary"/></div>
            ...
            {aiParsedData && aiParsedData.length > 0 && (
                <div className="p-4 border-t border-border flex-shrink-0">
                     <button 
                        onClick={handleApply} 
                        className="w-full text-center py-2.5 rounded-lg bg-green-600 text-white hover:bg-green-700 font-semibold transition-colors disabled:bg-muted"
                        disabled={isLoading}
                    >
            ...
            {/* Input Form */}
            <form onSubmit={handleSend} className="p-4 border-t border-border flex-shrink-0 flex items-center gap-3">
                <textarea
                    value={input}
                    ...
                    className="flex-grow p-2.5 rounded-lg border border-border bg-background focus:ring-2 focus:ring-primary/50 focus:outline-none resize-none"
                    rows={1}
                    disabled={isLoading}
                />
                <button type="submit" disabled={isLoading || !input.trim()} className="p-3 rounded-full bg-primary text-primary-foreground disabled:bg-muted hover:bg-primary/90 transition-colors">
