...
const DetailCard: React.FC<{ title: string; children: React.ReactNode; colorClass?: string }> = ({ title, children, colorClass = 'border-border' }) => (
                ...
                return (
                    <ul className="space-y-1.5 pl-0 list-none">
                        {value.map((item, index) => (
                            <li key={index} className="p-2 bg-background/50 rounded">
                                {Object.entries(item).map(([key, val]) => (
                                    <div key={key} className="text-xs grid grid-cols-[auto,1fr] gap-x-2">
                                        <strong className="font-semibold text-muted-foreground whitespace-nowrap">{formatHeader(key)}:</strong>
        ...
        if (data.length === 0) {
            return (
                <DetailCard title="Query Results">
                    <p className="text-sm text-muted-foreground italic">The AI returned no results for this query.</p>
                </DetailCard>
            );
        }
        ...
        if (allValuesAreEmptyArrays) {
            return (
                <DetailCard title="AI Response">
                    <p className="text-sm text-muted-foreground italic">I couldn't find any specific data matching your request in the database.</p>
                </DetailCard>
            );
        }
        ...
        return (
            <div className="space-y-4 text-card-foreground">
                 {Object.keys(primitiveData).length > 0 && (
                    <div className={`p-4 border rounded-lg border-border grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-2`}>
                        {Object.entries(primitiveData).map(([key, value]) => (
                            <div key={key} className="text-center p-2">
                                <p className="text-sm font-semibold text-muted-foreground">{formatHeader(key)}</p>
                                <p className="text-3xl font-bold text-foreground">{String(value)}</p>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {Object.entries(complexData).map(([key, value]) => {
                    ...
                    if (value !== null && typeof value === 'object') {
                        return (
                            <DetailCard key={key} title={formatHeader(key)}>
                                <pre className="text-xs whitespace-pre-wrap bg-background/50 p-2 rounded">{JSON.stringify(value, null, 2)}</pre>
                            </DetailCard>
                        );
                    }
                    return null;
