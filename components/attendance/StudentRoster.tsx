...
export const StudentRoster: React.FC = () => {
    ...
    useEffect(() => {
        const loadModels = async () => {
            try {
                ...
                console.error("Failed to load face-api models for registration:", error);
                showToast("Error loading AI models. Face registration is unavailable.", 'error');
            }
        };
        loadModels();
    }, [showToast]);
    ...
    }, [registrationStatus, currentStepIndex, scanSteps]);

    const handleRegisterClick = (studentId: string) => {
        if (!modelsLoaded) {
            showToast("AI models are still loading, please wait.", "info");
            return;
        }
        setScanProgress(0);
        setCurrentStepIndex(0);
    ...
    return (
        <>
            {registrationCandidate && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4" onClick={cancelRegistration}>
                    <div className="bg-card/80 dark:bg-card/70 backdrop-blur-lg border border-border rounded-2xl shadow-soft-xl p-8 w-full max-w-md text-center relative" onClick={e => e.stopPropagation()}>
                        <h2 className="text-2xl font-bold text-foreground">Registering {registrationCandidate.name}</h2>
                        
                        <PoseIndicator steps={scanSteps} currentStepIndex={currentStepIndex} />

                        <div className="relative w-full max-w-xs aspect-square mx-auto rounded-full overflow-hidden border-4 border-muted">
                        ...
                        <div className="px-4 mt-4"><div className="w-full bg-muted rounded-full h-2.5 mb-2"><div className="bg-primary h-2.5 rounded-full" style={{ width: `${scanProgress}%`, transition: 'width 0.5s ease-in-out' }}></div></div></div>
                        <div className="h-8 flex items-center justify-center">
                            {registrationStatus === 'success' ? <div className="flex items-center gap-3 text-success"><CheckCircleIcon className="h-8 w-8" /><p className="text-xl font-bold">{scanMessage}</p></div>
                             : (registrationStatus === 'scanning' || registrationStatus === 'saving') ? <div className="flex items-center gap-2"><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-current"></div><p className="text-lg text-muted-foreground">{scanMessage}</p></div>
                             : <p className="text-lg text-muted-foreground">{scanMessage}</p>}
                        </div>
                        <div className="mt-6 space-y-2">
                            {registrationStatus === 'scanned' && <button onClick={handleSaveRegistration} className="w-full h-12 px-4 rounded-lg bg-primary text-primary-foreground font-semibold hover:bg-primary/90">Save Registration</button>}
                            <button onClick={cancelRegistration} disabled={registrationStatus === 'scanning' || registrationStatus === 'saving'} className="w-full h-12 px-4 rounded-lg bg-muted text-muted-foreground font-semibold hover:bg-border disabled:opacity-50 disabled:cursor-not-allowed">{registrationStatus === 'scanned' ? 'Cancel' : 'Close'}</button>
                        </div>
                    </div>
                </div>
            )}
            <div className="w-full flex flex-col" style={{ height: 'calc(100vh - 12rem)' }}>
                <div className="bg-card p-4 rounded-xl shadow-soft border border-border mb-4 flex-shrink-0">
                    <h3 className="text-xl font-bold text-foreground mb-3">Student Roster</h3>
                    <div className="space-y-3">
                        <input type="text" placeholder="Search student..." value={filters.searchQuery} onChange={e => setFilters(f => ({ ...f, searchQuery: e.target.value }))} className="block w-full h-10 px-3 rounded-lg border border-border bg-background" />
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <select value={filters.grade} onChange={e => setFilters(f => ({ ...f, grade: e.target.value }))} className="block w-full h-10 px-3 rounded-lg border border-border bg-background"><option value="">All Grades</option>{[...new Set(students.map(s => s.grade))].sort().map(g => <option key={g} value={g}>{g}</option>)}</select>
                            <select value={filters.batch} onChange={e => setFilters(f => ({ ...f, batch: e.target.value }))} className="block w-full h-10 px-3 rounded-lg border border-border bg-background"><option value="">All Batches</option>{[...new Set(students.map(s => s.batch))].sort().map(b => <option key={b} value={b}>{b}</option>)}</select>
                            <select value={filters.registrationStatus} onChange={e => setFilters(f => ({ ...f, registrationStatus: e.target.value as any }))} className="block w-full h-10 px-3 rounded-lg border border-border bg-background"><option value="All">All Statuses</option><option value="Registered">Registered</option><option value="Unregistered">Unregistered</option></select>
                            <button onClick={clearFilters} className="h-10 px-4 rounded-lg bg-muted hover:bg-border text-sm font-medium">Clear</button>
                        </div>
                    </div>
                </div>
                <div className="flex-grow overflow-y-auto thin-scrollbar -mr-2 pr-2">
                    <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
                        {filteredRoster.length > 0 ? filteredRoster.map(s => <AttendanceStudentCard key={s.id} student={s} onViewHistory={() => setViewingStudentId(s.id)} onRegister={() => handleRegisterClick(s.id)} isRegistering={registrationCandidateId === s.id} registrationDisabled={!modelsLoaded}/>)
                        : <div className="col-span-full text-center py-16 text-muted-foreground"><p className="font-semibold text-lg">No students match filters.</p><p className="text-sm">Try adjusting your filters.</p></div>}
                    </div>
                </div>
            </div>
        </>
