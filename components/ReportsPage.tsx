...
const ReportsPage: React.FC = () => {
    ...
    const [studentFilters, setStudentFilters] = useState({ searchQuery: '', strongestSubject: '' });
    ...
    const studentPerformanceData = useMemo(() => {
        ...
        activeStudents.forEach(student => {
            ...
            if (completedStudentTests.length > 0) {
                ...
                completedStudentTests.forEach(t => {
                    if (t.marksObtained != null && t.totalMarks != null && t.totalMarks > 0) {
                        ...
                        scoresBySubject[t.subject].total += (t.marksObtained / t.totalMarks) * 100;
            ...
            data.set(student.id, { avgScore, strongestSubject, completedTests: completedStudentTests.length, upcomingTests: studentTests.filter(t => t.status === 'Upcoming').length, absentTests: studentTests.filter(t => t.status === 'Absent').length });
        });
        return data;
    }, [activeStudents, tests]);

    const allSubjectsForFilter = useMemo(() => Array.from(new Set(tests.map(test => test.subject))).sort(), [tests]);
    ...
    const clearFilters = useCallback(() => setStudentFilters({ searchQuery: '', strongestSubject: '' }), []);
    ...
    const stats = useMemo(() => (selectedStudentId && studentPerformanceData.has(selectedStudentId)) ? studentPerformanceData.get(selectedStudentId)! : { avgScore: 0, strongestSubject: 'N/A', completedTests: 0, upcomingTests: 0, absentTests: 0 }, [selectedStudentId, studentPerformanceData]);
        ...
        <div>
            {!selectedStudent ? (
                <>
                    ...
                    <ReportsFilterBar filters={studentFilters} onFilterChange={handleFilterChange} onClearFilters={clearFilters} allSubjects={allSubjectsForFilter} />
